 ...
 * FIX MINIMAL: Hapus loading block, gunakan hardcoded menu
 */
...
const Navbar = () => {
  ...
  // Custom hooks - TAPI IGNORE LOADING STATE
  const navData = useNavigation('header') || {}; 
  const configData = useSiteConfig() || {};
  
  // HARDCODED MENU - PASTIKAN SELALU ADA
  const menuItems = [
    {
      id: 1,
      title: 'Beranda',
  ...
  // Site branding - gunakan fallback jika API error
  const siteName = configData.siteSettings?.site_name?.value || 'LOGISTIK KITA';
  const logoUrl = configData.siteSettings?.site_logo?.value || '';
  const primaryColor = configData.siteSettings?.primary_color?.value || '#3B82F6';
  ...
  // REMOVE DEBUG LOGS untuk production
  // useEffect(() => {
  //   console.log('🔍 NAVBAR DEBUG:');
  //   console.log('navLoading:', navData.loading);
  //   console.log('settingsLoading:', configData.loading);
  // }, [navData.loading, configData.loading]);
  
  return (
    <>
      {/* Main Navbar - RENDER LANGSUNG TANPA TUNGGU LOADING */}
      <motion.nav
        initial={{ y: -100 }}
        animate={{ y: 0 }}
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-3">
                ...
                <motion.div whileHover={{ scale: 1.02 }} className="hidden sm:block">
                  <a href="/" className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-purple-400">
                    {siteName}
                  </a>
                </motion.div>
              </div>
            ...
            {/* Desktop Navigation - LANGSUNG RENDER */}
            <div className="hidden lg:flex items-center justify-center flex-1">
              <DesktopNav menuItems={menuItems} primaryColor={primaryColor} />
            </div>
